   ...
   */
	...
	for (int p = 0; p < this->particle_list.size(); p++) // for all particles in the domain
	{
		...
		if (this->particle_list[p].x[0] > (this->min_x[0] + 2 * this->h - dx) \
			...
			&& particle_list[p].x[1] > particle_list[p].x[0] * this->slope + this->b)
		{
			//update this->min_tf
				                        ...
				                        //for a single particle, using derivatives computed above
			{
				...
				else if (this->particle_list[p].x[1] + dt * this->particle_list[p].v[1] < (particle_list[p].x[0] + dt * this->particle_list[p].v[0]) * this->slope + this->b)
				{
					double new_v_0 = this->particle_list[p].v[0] + dt * this->particle_list[p].dedv[0]; 
					double new_v_1 = this->particle_list[p].v[1] + dt * this->particle_list[p].dedv[1]; 
					this->particle_list[p].v[0] = new_v_0;
					this->particle_list[p].v[0] = new_v_1;
					continue;
...
bool SPH_main::check_inside_domain(SPH_particle* part) {

	if (part->x[1] > part->x[0] * this->slope + this->b)
		return true;
	else
		return false;
